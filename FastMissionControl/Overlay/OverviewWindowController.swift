...
final class OverviewWindowController {
    ...
    func markWindowGone(_ windowID: CGWindowID) {
        ...
        // Re-flow the remaining windows into the freed space, then animate the closed card away and
        // the survivors to their new frames.
        let remaining = allWindowDescriptors.filter { !goneWindowIDs.contains($0.id) }
    ...
    private func setFilterText(_ text: String) {
        filterText = text
        let tokens = filterTokens(text)
        let matching: Set<CGWindowID>? = tokens.isEmpty
            ? nil
            : Set(allWindowDescriptors.filter { windowMatches($0, tokens: tokens) }.map(\.id))
    ...
    private func applyHideLayout(matching: Set<CGWindowID>?, displayText: String) {
        let visible = allWindowDescriptors.filter { descriptor in
            !goneWindowIDs.contains(descriptor.id) && (matching == nil || matching!.contains(descriptor.id))
        }
