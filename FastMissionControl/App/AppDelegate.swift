...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        menuBarController = MenuBarController(
            ...
            isLaunchAtLoginEnabled: { [weak self] in self?.settings.launchAtLogin ?? false },
            onLaunchAtLoginChanged: { [weak self] enabled in
                self?.appModel.requestLaunchAtLoginChange(enabled: enabled)
            }
        )
    ...
    private func applyStartupWindowBehavior() {
        ...
        if isFirstLaunch || !appModel.permissions.isReady {
            _ = appModel.showControlWindow()
        } else {
